...
describe('production DSH dependencies', () => {
  ...
  it('pins the session-query contract used by the history controller', async () => {
    ...
    expect(manifest.dependencies['@deepseek-ai/dsh-session-query']).toBe('0.1.2-alpha.5')
    expect(manifest.dependencies['@deepseek-ai/dsh-session-query-sqlite']).toBe('0.1.2-alpha.5')
    expect(manifest.dependencies['@deepseek-ai/dsh-session-persistence']).toBe('0.1.2-alpha.5')
  })
})
